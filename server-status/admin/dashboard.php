...
if (isset($_GET['new']) && 'incident' === $_GET['new']) {
    Incident::add();
}
...
?>
          ...
          <?php
         $post_services = isset($_POST['services']) && !is_array($_POST['services']) ? [$_POST['services']] : [];

foreach ($services as $service) {
    ?>
            <div class="item clearfix">
              <div class="service"><?php if (-1 !== $service->get_status()) {?><input type="checkbox" name="services[]" value="<?php echo $service->get_id(); ?>" <?php echo (in_array($service->get_id(), $post_services, true)) ? 'checked' : ''; ?> id="service-<?php echo $service->get_id(); ?>"><?php } ?><label for="service-<?php echo $service->get_id(); ?>"><?php echo $service->get_name(); ?></label></div>
              <div class="status <?php if (-1 !== $service->get_status()) {
                  echo $classes[$service->get_status()];
              }?>"><?php if (-1 !== $service->get_status()) {
