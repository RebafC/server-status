...
require_once __DIR__ . '/classes/locale-negotiator.php';

require_once __DIR__ . '/classes/db-class.php';

$isDeveleoperEnvironement = false;
if (isset($_GET['isDev']) && 'devMode' === $_GET['isDev']) {
...
if (isset($_POST['server'])) {
    ...
    if (isset($_POST['url']) && false === filter_var($_POST['url'], FILTER_VALIDATE_URL)) {
...
if (isset($_POST['server']) && '' === $message) {
    ...
    foreach ($array as $value) {
        ...
        if (false === $q_res) {
            $message = sprintf(_('Error while creating database. Please check permission for your account ' .
                'or MYSQL version.<br>Error: %s'), $mysqli->error);
    ...
    if ('' === $message) {
        ...
        $policy_url_conf = (empty($_POST['policy_url']))
            ? $_POST['url'] . '/policy.php'
            : htmlspecialchars($_POST['policy_url'], ENT_QUOTES);
        $config = str_replace('##policy_url##', $policy_url_conf, $config);

        file_put_contents('config.php', $config);

        include_once __DIR__ . '/create-server-config.php';
...
<summary><?php echo _('If any of the following prerequisites are shown as failed (red X),
    please correct the issue and reload the page before proceeding with the installation.'); ?></summary>
<?php

    $php_version_req = sprintf(_('Minimum PHP version %s'), MINIMUM_PHP_VERSION);
$preq_fail = ['remove', 'danger'];
$preq_ok = ['ok', 'success'];
...
if (strnatcmp(PHP_VERSION, MINIMUM_PHP_VERSION) >= 0) {
    $preq_phpver = $preq_ok;
}
...
<form method="post" action="." class="clearfix install">
	<section class="install-section clearfix">
		...
		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="servername"><?php echo _('Name'); ?>: </label>
                <input type="text" name="servername" value="<?php echo (isset($_POST['servername'])) ? htmlspecialchars($_POST['servername'], ENT_QUOTES) : ''; ?>" id="servername" placeholder="<?php echo _('Name'); ?>" class="form-control" required>
            </div>
			<div class="col-sm-6"><label for="url"><?php echo _('Url'); ?>: </label>
		...
		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="title"><?php echo _('Title'); ?>: </label>
                <input type="text" name="title" value="<?php echo (isset($_POST['title'])) ? htmlspecialchars($_POST['title'], ENT_QUOTES) : 'Server Status'; ?>" id="title" placeholder="<?php echo _('Title'); ?>" class="form-control" required>
            </div>
			<div class="col-sm-6"></div>
		</div>
		<summary><?php echo _('Also an email address for mailer would be nice :)'); ?></summary>
		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="mailer"><?php echo _('Name'); ?>: </label>
                <input type="text" name="mailer" value="<?php echo (isset($_POST['mailer'])) ? htmlspecialchars($_POST['mailer'], ENT_QUOTES) : ''; ?>" id="mailer" placeholder="<?php echo _('Name'); ?>" class="form-control" required>
            </div>
			<div class="col-sm-6"><label for="mailer_email"><?php echo _('Email'); ?>: </label>
	...
	<section class="install-section clearfix">
		...
		<summary><?php echo _('We need database connection to be able to create tables. ' .
            'Please check that your account has the permission needed to do that.'); ?></summary>

		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="server"><?php echo _('Server'); ?>: </label>
			...
			<div class="col-sm-6"><label for="database"><?php echo _('Database'); ?>: </label>
		...
		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="dbuser"><?php echo _('User'); ?>: </label>
			...
			<div class="col-sm-6"><label for="dbpassword"><?php echo _('Password'); ?>: </label>
	...
	<section class="install-section clearfix">
		...
		<summary><?php echo _('Since you are collecting personal information, ' .
            'the GDPR needs you to have a privacy policy. Enter the details below.'); ?></summary>

		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="policy_name"><?php echo _('Name'); ?>: </label>
			...
			<div class="col-sm-6"><label for="address"><?php echo _('Address'); ?>: </label>
		...
		<div class="form-group clearfix">
			<div class="col-sm-6"><label for="policy_mail"><?php echo _('E-Mail'); ?>: </label>
			...
			<div class="col-sm-6"><label for="policy_phone"><?php echo _('Phone'); ?>: </label>
	...
	<section class="install-section clearfix">
		...
		<summary><?php echo _("And finally, we need info to create a new user.' .
            ' You don't have to provide it, but then... No status page admin..."); ?></summary>
		<div class="form-group">
			<div class="col-sm-6"><label for="name"><?php echo _('Name'); ?>: </label>
			...
			<div class="col-sm-6"><label for="surname"><?php echo _('Surname'); ?>: </label>
		...
		<div class="form-group">
			<div class="col-sm-6"><label for="username"><?php echo _('Username'); ?>:</label>
			...
			<div class="col-sm-6"><label for="email"><?php echo _('Email'); ?>:</label>
		...
		<div class="form-group">
			<div class="col-sm-6"><label for="password"><?php echo _('Password'); ?>:</label>
