 ...
 * @param mixed      $c
 * @param mixed      $index
 * @param null|mixed $bytes
 */
function ordUTF8(array $c, $index = 0, &$bytes = null)
{
    $len = strlen($c);
    $bytes = 0;
    if ($index >= $len) {
 ...
 * @license http://opensource.org/licenses/LGPL-2.1 LGPL v2.1
 *
 * @see http://belousovv.ru/myscript/phpIDN
 */
function EncodePunycodeIDN($value)
{
    ...
    while (mb_strlen($str, 'UTF-8') > 0) {
        $input[] = mb_substr($str, 0, 1, 'UTF-8');
        $str = (PHP_VERSION_ID < 50408) ? mb_substr($str, 1, mb_strlen($str, 'UTF-8'), 'UTF-8') : mb_substr($str, 1, null, 'UTF-8');
    }
...
function DecodePunycodeIDN($value)
{
    ...
    while ($d < strlen($value)) {
        ...
        // http://tools.ietf.org/html/rfc3492#section-6.1
        $delta = (0 === $old_i) ? $delta / 700 : $delta >> 1;

        $count_output_plus_one = count($output) + 1;
        $delta += (int) ($delta / $count_output_plus_one);
