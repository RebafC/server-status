...
class subscriptions
{
    public function add($userID, $service): bool
    {
        global $mysqli;
    ...
    public function remove($userID, $service): bool
    {
        global $mysqli;
    ...
    public function render_subscribed_services($typeID, $subscriberID, string $userID, $token): void
    {
        global $mysqli;
        ...
        $subs = [];    // Will be used to hold IDs of services already selected
        ...
        $query = $mysqli->query(sprintf('SELECT services.id, services.name from services WHERE services.id %s', $subsExp));
        echo '<div class="list-group">';
        if ($query->num_rows) {
            while ($result = $query->fetch_assoc()) {
