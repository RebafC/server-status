...
{
    private array $accepted_langs = [];

    private $default_language;

    private array $all_locales = [
        'af_ZA' => 'Afrikaans',
        'am_ET' => 'አማርኛ',
        'ar_AE' => 'العربية',
        'ar_BH' => 'العربية',
     ...
     */
    public function get_accepted_langs(): array
    {
    ...
    public function negotiate($override = null)
    {
        ...
        if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
            ...
            if ((is_countable($lang_parse[1]) ? count($lang_parse[1]) : 0) > 0) {
        ...
        foreach ($langs as $lang) {
            if (strlen($lang) > 2) {
                if (in_array($lang, $accepted_langs, true)) {
                    $best_match = $lang;

                    break;
                }
            } else {
                $possible = array_filter($accepted_langs, static function ($key): bool {
                    global $lang;

                    return str_starts_with($key, $lang);
                });
