...
class service implements JsonSerializable
{
     ...
     */
    public function __construct($id, $name, $description = null, $group_name = '', $status = 3)
    {
    ...
    public function render(): void
    {
        global $statuses;
        global $classes;
        static $arrCompletedGroups = [];
        // static $boolClosed;
        static $boolOpened;

        // Check if previous ul has been opened, and if a empty/new group is being
        // render_header, close the UL first.
        if ($boolOpened && (empty($this->group_name) || !in_array($this->group_name, $arrCompletedGroups, true))) {
            echo '</ul>';
            $boolOpened = false;
        }

        // If no group exist or group is new, start a new UL
        if (!empty($this->group_name) && !in_array($this->group_name, $arrCompletedGroups, true)) {
