...
class incident implements JsonSerializable
{
    ...
    public static function add(): void
    {
        ...
        if (2 === $type && (!strlen(trim($_POST['time'])) || !strlen(trim($_POST['end_time'])))) {
        ...
        if (empty($_POST['services'])) {
            ...
            if (!empty($_POST['time']) && 2 === $type) {
                $input_time = (empty($_POST['time_js']) ? $_POST['time'] : $_POST['time_js']);
                $input_end_time = (empty($_POST['end_time_js']) ? $_POST['end_time'] : $_POST['end_time_js']);
            ...
            $notification->populate_impacted_services($status_id);

            $notification->type = $type;
            $notification->time = $time;
            $notification->title = $title;
            $notification->text = $text;
            $notification->status = $statuses[$type];
     ...
     * Renders incident.
     *
     * @param bool $admin - decides whether admin controls should be rendered
     */
    public function render($admin = 0): void
    {
        global $icons;
        global $classes, $user;
